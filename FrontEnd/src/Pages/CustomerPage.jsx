...
import { FaEnvelope, FaLock, FaSignInAlt, FaUserPlus, FaSpinner, FaReceipt, FaShieldAlt, FaCloudUploadAlt } from 'react-icons/fa';
...
const CustomerPage = () => {
    ...
    const { login, isLoading: isAuthLoading, isCustomerAuth, handleGoogleAuth, customerUser } = useAuth();
    ...
    useEffect(() => {
        if (isCustomerAuth && customerUser?.role === 'customer') {
            navigate('/customerDashboard');
        }
    }, [isCustomerAuth, customerUser, navigate]);
            ...
            <div className="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-center p-16 overflow-hidden"
                ...
                <div className="relative z-10 max-w-md text-center">
                    ...
                    <h1 className="text-4xl font-black text-white mb-3 leading-tight">
                        Welcome to <span className="text-orange-400">BillEase</span>
                    </h1>
                    <p className="text-indigo-200 text-lg mb-10">Your smart digital bill & warranty manager</p>
                ...
                <div className={`w-full max-w-md transition-all duration-700 ${isPageLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                    ...
                    <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 sm:p-10">
                        <div className="mb-8">
                            ...
                            <p className="text-gray-500 text-sm mt-1">Welcome back! Enter your details below.</p>
                        </div>

                        <form className="space-y-5" onSubmit={handleEmailPasswordSubmit}>
                            <div>
                                <label htmlFor="email-login" className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Email Address</label>
                                <div className="relative">
                                    <FaEnvelope className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm" />
                                    <input id="email-login" type="email" placeholder="you@example.com" required value={email}
                            ...
                            <div>
                                <label htmlFor="password-login" className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Password</label>
                                <div className="relative">
                                    <FaLock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm" />
                                    <input id="password-login" type="password" placeholder="••••••••" required value={password}
                            ...
                            <button type="submit" disabled={isAuthLoading}
                                ...
                                {isAuthLoading ? <><FaSpinner className="animate-spin" /> Signing in...</> : <><FaSignInAlt /> Sign In</>}
                            </button>
                        </form>
                        ...
                        <p className="text-center text-sm text-gray-500 mt-6">
                            No account yet?{' '}
                            <NavLink to="/CustomerRegister" className="font-bold text-orange-600 hover:text-orange-700 hover:underline inline-flex items-center gap-1">
