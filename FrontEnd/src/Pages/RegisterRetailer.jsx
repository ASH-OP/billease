...
import {
    FaSpinner, FaEnvelope, FaLock, FaUserTie, FaStore,
    FaFileInvoice, FaUsers, FaChartLine, FaShieldAlt, FaCheckCircle
} from 'react-icons/fa';
...
const RetailerRegister = () => {
    ...
    const { register, isLoading: isAuthLoading, handleGoogleAuth } = useAuth();
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const { data } = await customerApi.post('/auth/send-otp', { email, name: ownerName });
            if (data.success) {
    ...
    const handleOtpVerified = async (token) => {
        ...
        if (success) {
            toast.success('Account created! Complete your shop details.');
    ...
    const handleGoogleSuccess = async (credentialResponse) => {
        try {
            ...
            if (response?.success) {
                const hasBusinessDetails = response.user?.shopName &&
                    (response.user?.gstNumber || response.user?.panNumber);
                navigate(hasBusinessDetails ? '/retailerDashboard' : '/completeRetailerProfile');
            }
        } catch {
                ...
                <div className="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-center p-16 overflow-hidden"
                    ...
                    <div className="relative z-10 max-w-md text-center">
                        ...
                        <h1 className="text-4xl font-black text-white mb-3 leading-tight">
                            Join as a <span className="text-orange-400">Retailer</span>
                        </h1>
                        <p className="text-slate-300 text-lg mb-10">Set up your store on BillEase in minutes</p>
                    ...
                    <div className={`w-full max-w-md transition-all duration-700 ${isPageLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
                        ...
                        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 sm:p-10">
                            <div className="mb-8">
                                ...
                                <p className="text-gray-500 text-sm mt-1">We'll verify your email before creating your account.</p>
                            </div>

                            <form className="space-y-5" onSubmit={handleSubmit}>
                                {/* Owner Name */}
                                <div>
                                    <label htmlFor="ownerName" className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                                        Owner Full Name
                                    </label>
                                    <div className="relative">
                                        <FaUserTie className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm" />
                                        <input
                                            id="ownerName" type="text" placeholder="e.g. Rajesh Kumar"
                                ...
                                {/* Email */}
                                <div>
                                    <label htmlFor="reg-email" className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                                        Business Email
                                    </label>
                                    <div className="relative">
                                        <FaEnvelope className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm" />
                                        <input
                                            id="reg-email" type="email" placeholder="store@example.com"
                                ...
                                {/* Password */}
                                <div>
                                    <label htmlFor="reg-password" className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                                        Password
                                    </label>
                                    <div className="relative">
                                ...
                                {/* Confirm Password */}
                                <div>
                                    <label htmlFor="confirm-password" className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                                        Confirm Password
                                    </label>
                                    <div className="relative">
                                        <FaLock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm" />
                            ...
                            <div className="flex justify-center">
                                <GoogleLogin
                                    ...
                                    useOneTap theme="outline" size="large" shape="rectangular"
                                    width="320px" disabled={isLoading}
                                />
                            </div>

                            <p className="text-center text-sm text-gray-500 mt-6">
                                Already have an account?{' '}
                                <NavLink to="/retailerLogin" className="font-bold text-orange-600 hover:text-orange-700 hover:underline inline-flex items-center gap-1">
                                    <FaStore size={11} /> Sign in to Dashboard
                                </NavLink>
                            </p>
                        </div>
                    </div>
