...
import { FaSpinner, FaUser, FaEnvelope, FaLock, FaReceipt, FaShieldAlt, FaCloudUploadAlt, FaUserPlus } from 'react-icons/fa';
        ...
        <div className={`min-h-screen flex transition-opacity duration-500 ${isPageLoaded ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="hidden lg:flex lg:w-1/2 relative flex-col items-center justify-center p-16 overflow-hidden"
                ...
                <div className="relative z-10 max-w-md text-center">
                    ...
                    <p className="text-indigo-300/70 text-xs mt-10">Free forever · No credit card required</p>
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 sm:p-10">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input type="text" placeholder="John Doe" required value={name}
                                        onChange={e => setName(e.target.value)} disabled={isAuthLoading} className={inputClass} />
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input type="email" placeholder="you@example.com" required value={email}
                                        onChange={e => setEmail(e.target.value)} disabled={isAuthLoading} className={inputClass} />
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input type="password" placeholder="Min. 6 characters" required value={password}
                                        onChange={e => setPassword(e.target.value)} disabled={isAuthLoading} className={inputClass} />
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input type="password" placeholder="Re-enter password" required value={confirmPassword}
                                        onChange={e => setConfirmPassword(e.target.value)} disabled={isAuthLoading} className={inputClass} />
                                </div>
                            </div>

                            <button type="submit" disabled={isAuthLoading}
                                className={`w-full flex justify-center items-center gap-2.5 py-3.5 px-6 rounded-xl font-bold text-white bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 shadow-lg shadow-orange-200 hover:shadow-orange-300 transition-all text-sm mt-2 ${isAuthLoading ? 'opacity-70 cursor-wait' : 'hover:-translate-y-px active:translate-y-0'}`}>
                                {isAuthLoading ? <><FaSpinner className="animate-spin" /> Creating account...</> : <><FaUserPlus /> Create Account</>}
                            </button>
                        </form>
                        ...
                        <div className="flex justify-center">
                            <GoogleLogin onSuccess={handleGoogleSuccess} onError={handleGoogleError}
                                useOneTap theme="outline" size="large" width="320px" disabled={isAuthLoading} />
                        </div>
